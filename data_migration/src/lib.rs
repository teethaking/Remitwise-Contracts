...
mod tests {
    ...
    fn test_algorithm_field_roundtrips_json() {
        let snapshot = ExportSnapshot::new(sample_remittance_payload(), ExportFormat::Json);
        let bytes = export_to_json(&snapshot).unwrap();
        let loaded = import_from_json(&bytes, &mut MigrationTracker::new(), 123456).unwrap();
        assert_eq!(loaded.header.hash_algorithm, ChecksumAlgorithm::Sha256);
    }
    ...
    fn test_algorithm_field_roundtrips_binary() {
        ...
        let bytes = export_to_binary(&snapshot).unwrap();
        let loaded = import_from_binary(&bytes, &mut MigrationTracker::new(), 123456).unwrap();
        assert_eq!(loaded.header.hash_algorithm, ChecksumAlgorithm::Sha256);
    }
    ...
    fn test_e2e_savings_snapshot_encrypted_roundtrip() {
        ...
        let loaded = import_from_json(&decoded_bytes, &mut MigrationTracker::new(), 123456).unwrap();
        assert_eq!(loaded.header.version, SCHEMA_VERSION);
        assert!(loaded.verify_checksum());
    ...
    fn test_e2e_locked_goal_preserved_json_and_binary() {
        ...
        let json_loaded = import_from_json(&json_bytes, &mut MigrationTracker::new(), 123456).unwrap();
        ...
        let bin_loaded = import_from_binary(&bin_bytes, &mut MigrationTracker::new(), 123456).unwrap();
